...
class UpdateBranchHourRequest extends FormRequest
{
    use ApiResponse;
    ...
    public function rules(): array
    {
        return [
            'day_of_week' => [
                'sometimes', 'required', 'integer', 'between:0,6',
                Rule::unique('branch_hours', 'day_of_week')
                    ->where('branch_id', $this->route('branch')?->id)
                    ->ignore($this->route('hour')),
            ],
