  ...
  <v-card rounded="xl" elevation="0" border class="mb-4">
    <v-card-title class="pa-5 pb-3">
      <div class="d-flex align-center justify-space-between">
        <div>
          <div class="text-subtitle-1 font-weight-bold">{{ $t('dashboard.revenue_overview.title') }}</div>
          <div class="text-caption text-medium-emphasis">{{ $t('dashboard.revenue_overview.subtitle') }}</div>
        </div>
        <v-btn-toggle
          v-model="chartMode"
        ...
        >
          <v-btn value="revenue" size="small">{{ $t('dashboard.revenue_overview.tabs.revenue') }}</v-btn>
          <v-btn value="orders"  size="small">{{ $t('dashboard.revenue_overview.tabs.orders') }}</v-btn>
        </v-btn-toggle>
      </div>
    </v-card-title>
        ...
        tooltip: {
          ...
          callbacks: {
            label: ctx => chartMode.value === 'revenue'
              ...
              : ` ${ctx.parsed.y} ${t('dashboard.revenue_overview.tabs.orders')}`,
          },
        },
      },
      scales: {
