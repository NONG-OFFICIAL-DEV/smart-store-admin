<template>
  <v-container fluid class="pa-0">
    <custom-title>
      ...
      <strong>{{ isEdit ? t('purchases.form.edit_title') : t('purchases.form.add_title') }}</strong>
      <template #right>
        <v-switch
          :label="t('purchases.form.use_ai_suggestions')"
          inset
          width="230"
          hide-details
          :false-value="false"
    ...
    <v-form ref="formRef" v-model="isValid">
      ...
      <v-card rounded="lg" elevation="0" class="pa-4 mb-4">
        <v-card-title class="text-h6 font-weight-bold pb-2">
          {{ t('purchases.form.purchase_information') }}
        </v-card-title>
        ...
        <v-row class="mt-4">
          <v-col cols="12" sm="6" md="3">
            <v-select
              ...
              item-title="name"
              item-value="id"
              :rules="[v => !!v || t('purchases.form.supplier_required')]"
            >
              <template #append-item>
                ...
                <v-list-item class="text-primary" @click="openCreateSupplier">
                  <v-list-item-title>
                    ...
                    {{ t('purchases.form.create_new_supplier') }}
                  </v-list-item-title>
                </v-list-item>
              </template>
            </v-select>
          ...
          <v-col cols="12" sm="6" md="3">
            <v-date-input
              ...
              :label="t('purchases.form.purchase_date')"
              :rules="[v => !!v || t('purchases.form.date_required')]"
              :min="today"
            />
          </v-col>
          <!-- <v-col cols="12" sm="6" md="3">
          ...
          <v-col cols="12" sm="6" md="3">
            <v-select
              :label="t('purchases.form.payment_status')"
      ...
      <v-card rounded="lg" elevation="0" class="pa-4">
        ...
        <div v-else>
              ...
              >
                ...
                <v-col cols="12" md="2">
                  <v-text-field
                    ...
                    :rules="[v => v >= 0 || t('purchases.form.price_non_negative')]"
                  />
                </v-col>
                <v-col cols="12" md="2">
                  <v-text-field
          ...
          <v-row>
            <v-col cols="12" sm="6" md="8">
              <v-textarea :label="t('form.notes')" v-model="purchase.note"></v-textarea>
            </v-col>
            <v-col cols="12" sm="8" md="4">
              <v-sheet border rounded="lg" class="pa-4">
                <div class="mb-2">{{ t('purchases.form.summary') }}</div>
                ...
                <div class="d-flex justify-space-between mb-1">
                  <span>{{ t('purchases.form.total_discount_amount') }}:</span>
                  <strong>{{ formatCurrency(totalDiscount) }}</strong>
                </div>

                <div class="d-flex justify-space-between mb-1">
                  <span>{{ t('purchases.form.total_tax_amount') }}:</span>
                ...
                <div class="d-flex justify-space-between">
                  <h3 class="font-weight-bold">{{ t('purchases.form.total_amount') }}:</h3>
        ...
        <v-row>
          <v-col>
            <v-btn text color="primary" @click="addItem">+ {{ t('po.add_product') }}</v-btn>
          </v-col>
          ...
          <v-col class="text-end">
            <v-btn
              ...
              class="me-2"
            >
              {{ isEdit ? t('purchases.form.update_draft') : t('purchases.form.save_draft') }}
            </v-btn>
            ...
            <v-btn
              ...
              :disabled="purchase.items.length <= 0 || !isValid"
            >
              {{ t('purchases.form.submit_request') }}
            </v-btn>
...
<script setup>
  ...
  const paymentStatusOptions = computed(() => [
    { title: t('purchases.form.payment_unpaid'), value: 'unpaid' },
    { title: t('purchases.form.payment_partial'), value: 'partial' },
    { title: t('purchases.form.payment_paid'), value: 'paid' }
  ])
      ...
      items.forEach((item, index) => {
        ...
        if (seen.has(item.product_id)) {
          ...
          confirm({
            ...
            message: t('purchases.form.duplicate_message', { name: product.name }),
  ...
  async function savePurchase(requestApproval = false) {
    ...
    if (isEdit.value && purchase.status === PURCHASE_STATUSES.RECEIVED) {
      return notif(t('purchases.form.cannot_edit_received'), {
        type: 'error',
        color: 'error'
      })
    }
    ...
    if (isEdit.value) {
      ...
      notif(
        requestApproval
          ? t('purchases.form.saved_and_requested')
  ...
  const approvePurchase = async () => {
    ...
    notif(t('purchases.form.approved'), { type: 'success', color: 'primary' })
    router.push('/purchases')
  }

  const rejectPurchase = async () => {
    await purchaseStore.updateStatus(purchase.id, 'rejected')
    notif(t('purchases.form.rejected'), { type: 'error', color: 'error' })
    router.push('/purchases')
  }
  ...
  async function fetchAISuggestions() {
    try {
      ...
      return notif(`${t('purchases.form.ai_fetch_failed')} ${error?.message ?? ''}`.trim(), {
        type: 'error',
        color: 'error'
      })
    }
