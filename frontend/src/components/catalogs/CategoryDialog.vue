  ...
  <v-dialog v-model="model" max-width="670" persistent scrollable>
    <v-card rounded="lg">
        ...
        <v-form ref="formRef" @submit.prevent="submit">
            ...
            <v-col cols="6">
              ...
              >
                <template #prepend-inner>
                  <v-icon icon="mdi-file-tree-outline" size="18" class="text-grey" />
                </template>
              </v-select>
              <p class="text-caption text-grey mt-1 ml-1">Leave empty for top-level</p>
            </v-col>
          </v-row>
          ...
          <v-row dense>
            <v-col cols="12">
              ...
              >
                <template #prepend-inner>
                  <v-icon icon="mdi-store-outline" size="18" class="text-grey" />
                </template>
                <template #chip="{ item, props: chipProps }">
                  <v-chip v-bind="chipProps" size="small" rounded="md" color="primary" variant="tonal">
                    {{ item.title }}
                  </v-chip>
                </template>
              </v-select>
          ...
          <v-row dense>
            <v-col cols="12" md="6">
              <label class="text-body-2 font-weight-medium text-grey-darken-2 mb-1 d-block">
                Icon <span class="text-caption text-grey ml-1">(MDI icon name)</span>
              </label>
              <v-text-field
                v-model="form.icon"
              ...
              >
                <template #prepend-inner>
                  <v-icon :icon="iconPreview" size="18" :color="form.color || 'grey'" />
                </template>
              </v-text-field>
            </v-col>
            ...
            <v-col cols="12" md="6">
              <label class="text-body-2 font-weight-medium text-grey-darken-2 mb-1 d-block">
                Color <span class="text-caption text-grey ml-1">(hex)</span>
              </label>
              <v-text-field
                v-model="form.color"
                placeholder="#FF5733"
              ...
              >
                <template #prepend-inner>
                  <div class="color-swatch" :style="{ background: colorPreview }" />
                </template>
              </v-text-field>
            </v-col>
          </v-row>
          ...
          <v-row dense>
            <v-col cols="12" md="6">
              <label class="text-body-2 font-weight-medium text-grey-darken-2 mb-1 d-block">
                Image URL <span class="text-caption text-grey ml-1">(optional)</span>
              </label>
              <v-text-field
              ...
              >
                <template #prepend-inner>
                  <v-icon icon="mdi-image-outline" size="18" class="text-grey" />
                </template>
          ...
          <v-row dense>
            <v-col cols="12">
              <div class="d-flex align-center justify-space-between pa-4 rounded-lg bg-grey-lighten-5">
                <div>
                  <p class="text-body-2 font-weight-medium text-grey-darken-2 mb-0">Active Status</p>
                  <p class="text-caption text-grey mb-0">Category will be visible when active</p>
                </div>
      ...
      <v-card-actions class="px-6 py-4 gap-3">
        <v-btn variant="outlined" rounded="lg" :disabled="loading" @click="close">
          Cancel
        </v-btn>
        <v-btn
...
const rules = {
  ...
  icon: [
    ...
    v => !v || /^mdi-[a-z0-9-]+$/.test(v) || 'Must be a valid MDI icon (e.g. mdi-food)'
  ],
  color: [
