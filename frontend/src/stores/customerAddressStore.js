...
export const useCustomerAddressStore = defineStore('customerAddress', {
  ...
  actions: {
    ...
    async fetchCustomerAddresses(customerId, filters) {
      ...
      this.customerAddresses = res.data.data.data ?? res.data.data
      this.pagination = res.data.data
    },

    async fetchCustomerAddressById(customerId, id) {
      const res = await getCustomerAddressByIdApi(customerId, id)
