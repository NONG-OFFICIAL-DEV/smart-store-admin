...
export const usePurchaseOrderStore = defineStore('purchaseOrder', {
  ...
  actions: {
    ...
    async submitPurchaseOrder(id) {
      const res = await submitPurchaseOrderApi(id)
      const index = this.purchaseOrders.findIndex(item => item.id === id)
      if (index !== -1) this.purchaseOrders[index] = res.data.data
    },
    async confirmPurchaseOrder(id) {
      const res = await confirmPurchaseOrderApi(id)
      const index = this.purchaseOrders.findIndex(item => item.id === id)
      if (index !== -1) this.purchaseOrders[index] = res.data.data
    },
    async receivePurchaseOrder(id, data) {
      const res = await receivePurchaseOrderApi(id, data)
      const index = this.purchaseOrders.findIndex(item => item.id === id)
      if (index !== -1) this.purchaseOrders[index] = res.data.data
    },
    async cancelPurchaseOrder(id) {
      const res = await cancelPurchaseOrderApi(id)
      const index = this.purchaseOrders.findIndex(item => item.id === id)
      if (index !== -1) this.purchaseOrders[index] = res.data.data
    }
  }
