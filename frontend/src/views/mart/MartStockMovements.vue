  ...
  <div>
    ...
    <div class="d-flex align-center justify-space-between mb-6">
      <div>
        ...
        <p class="text-caption text-medium-emphasis mt-1">Full stock ledger history</p>
      </div>
      <v-btn color="primary" variant="flat" rounded="lg"
        prepend-icon="mdi-tune" @click="adjustDialog = true">
        Adjust Stock
      </v-btn>
    </div>
        ...
        <v-row dense align="center">
          ...
          <v-col cols="12" sm="3">
            <v-autocomplete
              ...
              rounded="lg" hide-details clearable
          ...
          <v-col cols="6" sm="2">
            <v-text-field
              v-model="filters.from" type="date" label="From"
              variant="outlined" density="compact"
          ...
          <v-col cols="6" sm="2">
            <v-text-field
              v-model="filters.to" type="date" label="To"
              variant="outlined" density="compact"
              rounded="lg" hide-details
              @update:model-value="load" />
          </v-col>
          ...
          <!-- Clear -->
          <v-col cols="6" sm="1">
            <v-btn variant="tonal" rounded="lg" block
              :disabled="!hasFilters" @click="clearFilters">
              Clear
            </v-btn>
          </v-col>
        </v-row>
    ...
    <v-card rounded="lg" border elevation="0">
      <v-data-table-server
        ...
        @update:items-per-page="p => { filters.per_page = p; filters.page = 1; load() }"
      >
        <!-- Date -->
        <template #item.created_at="{ item }">
          <div class="text-body-2">{{ fmtDate(item.created_at) }}</div>
          <div class="text-caption text-medium-emphasis">{{ fmtTime(item.created_at) }}</div>
        </template>
        ...
        <template #item.product="{ item }">
          <div class="d-flex align-center gap-2 py-1">
            <v-avatar size="34" rounded="lg"
              ...
              <v-img v-if="item.product?.image_url"
                :src="item.product.image_url" cover />
        ...
        <template #no-data>
          <div class="text-center py-12">
            <v-icon icon="mdi-history" size="48"
              color="grey-lighten-2" class="mb-3" />
...
const headers = [
  ...
  { title: 'Before → After',key: 'qty_before',    sortable: false, width: '130' },
...
const summaryCards = computed(() => {
  ...
  return [
    {
      label: 'Total Entries', icon: 'mdi-history',
      color: 'primary',
    ...
    {
      label: 'Purchases',     icon: 'mdi-package-down',
      color: 'success',
      value: all.filter(m => m.movement_type === 'purchase').length,
    },
    {
      label: 'Adjustments',   icon: 'mdi-tune',
      color: 'warning',
      value: all.filter(m => ['adjustment_in','adjustment_out','waste','count'].includes(m.movement_type)).length,
    },
  ]
})
...
const fmtTime = v => new Date(v).toLocaleTimeString('en-US', {
