    ...
    <v-row dense align="center" class="mb-4">
      ...
      <v-col cols="12" sm="auto">
        ...
        >
          ...
          <v-btn value="starter" size="small" class="text-none px-3">
            Starter
          </v-btn>
          <v-btn value="pro" size="small" class="text-none px-3">Pro</v-btn>
          <v-btn value="enterprise" size="small" class="text-none px-3">
            Enterprise
          </v-btn>
        </v-btn-toggle>
      </v-col>
      ...
      >
        ...
        <template #item.plan="{ item }">
          <v-chip
            ...
            variant="tonal"
            size="small"
            :prepend-icon="planIcon(item.plan)"
          >
            {{ item.plan }}
          </v-chip>
        </template>
        ...
        <template #item.actions="{ item }">
          <div class="d-flex gap-1 justify-end">
            ...
            <!-- Suspend / Activate -->
            <v-tooltip
              :text="item.is_active ? 'Suspend tenant' : 'Activate tenant'"
            >
      ...
      <v-card rounded="xl" elevation="0" border>
        <v-card-text class="pa-6 text-center">
          ...
          <p class="text-body-2 text-medium-emphasis mb-4">
            Transfer
            <strong>{{ transferDialog.tenant?.name }}</strong>
            to a new owner
          </p>
          <v-text-field
        ...
        <v-card-actions class="px-6 pb-6 pt-0 gap-3">
          <v-btn
            ...
            variant="tonal"
            rounded="lg"
            @click="transferDialog.show = false"
          >
            Cancel
          </v-btn>
          <v-btn
...
<script setup>
  ...
  const confirmDelete = async tenant => {
    ...
    try {
      confirm({
        title: 'Delete Branch',
        message: `Are you sure you want to delete branch "${tenant.name}"?`,
